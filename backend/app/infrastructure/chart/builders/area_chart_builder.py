"""Area Chart Builder implementation."""
...
class AreaChartBuilder(BaseChartBuilder):
    """Builder for AREA_CHART visualizations."""
    ...
    def build(self, result: QueryResult, config: ChartConfiguration) -> ChartResult:
        """Build an Area Chart model from QueryResult and ChartConfiguration."""
        ...
        for row in result.rows:
            formatted_x = self._format_label(row.get(x_col))
        ...
        title = config.title or f"Area Chart ({', '.join(y_cols)})"
        subtitle = config.subtitle

        metadata = {
            "chart_type": ChartType.AREA_CHART.value,
            "x_axis": x_col,
            "y_axes": y_cols,
            "group_by": group_col,
