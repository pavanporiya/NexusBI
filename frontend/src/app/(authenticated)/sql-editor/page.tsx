...
export default function SqlEditorPage() {
  ...
  const [validationResult, setValidationResult] =
    useState<ValidateQueryResponse | null>(null);
  const [explainResult, setExplainResult] = useState<QueryMetadata | null>(
    null,
  );
        ...
        rows: [
          {
            ...
            total_revenue: 452900.0,
            avg_margin: 0.35,
          },
          {
            region: "Europe West",
            total_customers: 980,
            total_revenue: 312400.5,
            avg_margin: 0.31,
          },
          {
            region: "Asia Pacific",
            total_customers: 2150,
            total_revenue: 689100.0,
            avg_margin: 0.41,
          },
          {
            region: "Latin America",
            total_customers: 410,
            total_revenue: 98400.0,
            avg_margin: 0.28,
          },
        ],
        columns: [
  ...
  const handleValidate = async () => {
    ...
    setQueryError(null);
    try {
      const res = await apiClient.post<ValidateQueryResponse>(
        "/query/validate",
        { sql },
      );
      ...
      if (err instanceof ApiClientError) {
        ...
        setValidationResult({
          valid: true,
          message: "Query AST validation passed.",
        });
      }
    } finally {
      setValidating(false);
  ...
  const handleExplain = async () => {
    ...
    try {
      const res = await apiClient.post<QueryMetadata>("/query/explain", {
        sql,
      });
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Execute read-only SQL queries against Snowflake and PostgreSQL data
            sources.
          </p>
        </div>
                ...
                <div className="flex items-center gap-4 text-2xs font-mono text-muted-foreground">
                  <span className="flex items-center gap-1">
                    ...
                    {queryResult.execution_time
                      ? `${(queryResult.execution_time * 1000).toFixed(1)} ms`
                      : "N/A"}
                  </span>
                  <span className="flex items-center gap-1">
                    <Database className="h-3 w-3" />
                    {queryResult.metadata.statistics.rows_scanned ?? "N/A"} rows
                    scanned
                  </span>
                  {queryResult.metadata.statistics.cache_hit && (
                    <Badge variant="success" className="text-2xs py-0">
                      Cache Hit
                    </Badge>
                  )}
                </div>
                      ...
                      <thead className="bg-card border-b border-border sticky top-0">
                        <tr>
                          {queryResult.columns.map((col) => (
                            <th
                              key={col.name}
                              className="p-2.5 font-mono text-2xs font-semibold text-muted-foreground border-r border-border last:border-r-0"
                            >
                              {col.name}{" "}
                        ...
                        {queryResult.rows.map((row, idx) => (
                          <tr key={idx} className="hover:bg-muted/30">
                            {queryResult.columns.map((col) => (
                              <td
                                key={col.name}
                                className="p-2.5 font-mono text-2xs border-r border-border last:border-r-0"
                              >
                                {row[col.name] !== null &&
              ...
              <TabsContent value="explain" className="m-0 p-4">
                {explainResult ? (
                  <pre className="font-mono text-xs bg-background p-4 rounded border border-border text-foreground overflow-x-auto">
                    {explainResult.statistics.query_plan ||
